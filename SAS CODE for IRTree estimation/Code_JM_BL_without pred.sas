ods pdf file="C:\Users\leventbc\James Madison University\Research Team - Engagement - General\Student Documents\Juste Mehou\Brian work\output.pdf";
ods html close;
libname out 'C:\Users\leventbc\James Madison University\Research Team - Engagement - General\Student Documents\Juste Mehou\Brian work';
proc import out = dataset
  datafile= "C:\Users\leventbc\James Madison University\Research Team - Engagement - General\Student Documents\Juste Mehou\Brian work\pisa_usa_ready.csv"
  replace dbms=csv;
  guessingrows=max;
  Getnames=yes;
run;
...
proc mcmc 
   ...
   * Parms initiation statements for dichotomous items (2nd STAGE);
	...
	* Parms initiation statements for polytomous items (2nd STAGE);
    ...
    prior b: ~ normal (0, var=5);   *All parameters starting with a "b" will follow this distribution;
    ...
    /*Priors related to ability parameters*/
 		...
 		array theta[2] TOI DISENG;      *array to reference theta_TOI and theta_Diseng;
		array mu{2} mu_TOI mu_DISENG;   *array of the mean of the Multivariate_normal ( transprose of (mu_TOI, mu_Diseng));
		...
		*Define constants related to probability distribution of ability parameters;
		...
		mu_DISENG = 0;  *mean of theta_Disengagement in joint probability distribution (Multivariate normal distribution);
	...
	do j= 1 to &n_dichotomous;
			p_RG = logistic (a1 * (DISENG - b1_[j])); *probability of RG (stage 1);
