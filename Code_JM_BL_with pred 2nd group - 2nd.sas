...
proc mcmc 
	...
	*parms initiation statement for predictors coefficients;
		*1st group;
		...
		parms eta3 0;  *ESCS ;
		*2nd group;
		parms eta4_1 0; * Growth_disagree;
		parms eta4_2 0; * Growth_agree;
		parms eta4_3 0; * Growth_str_agree;
		parms eta5_1 0; * Grade_10;
		parms eta5_2 0; * Grade_9;
		parms eta6 0;  *EXPECEDU_BACHELORS;
		parms eta7 0;  *BELONG;
		parms eta8 0;  *FAMSUP;
		parms eta9 0;  *BULLIED;
   ...
   * Parms initiation statements for dichotomous items (2nd STAGE);
	...
	*All parameters starting with a "e" will follow this distribution;
	...
	prior eta4_1 ~ normal (0, var=4); * Growth_disagree;
	prior eta4_2 ~ normal (0, var=4); * Growth_agree;
	prior eta4_3 ~ normal (0, var=4); * Growth_str_agree;
	prior eta5_1 ~ normal (0, var=4); * Grade_10;
	prior eta5_2 ~ normal (0, var=4); * Grade_9;
	prior eta6 ~ normal (0, var=4);  *EXPECEDU_BACHELORS;
	prior eta7 ~ normal (0, var=4);  *BELONG;
	prior eta8 ~ normal (0, var=4);  *FAMSUP;
	prior eta9 ~ normal (0, var=4);  *BULLIED;
	...
	do j= 1 to &n_dichotomous;
			p_RG = logistic (a1 * (eta0 + eta1*Female + eta2*LANG_of_test + eta3*ESCS 
										+ eta4_1*Growth_disagree + eta4_2*Growth_agree + eta4_3*Growth_str_agree 
										+ eta5_1*Grade_10 + eta5_2*Grade_9 
           								+ eta6*EXPECEDU_BACHELORS + eta7*BELONG 
         							    + eta8*FAMSUP + eta9*BULLIED + DISENG_RES - b1_[j])); *probability of RG (stage 1);
